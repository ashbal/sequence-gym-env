...
class SeqEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if 'J' in card and newSequences and self.game.legalMove > 0:
            reward[self.game.currentPlayer] = reward[self.game.currentPlayer] + newSequences

        if (card == 'J♠' and self.game.legalMove) or (card =='J♥' and self.game.legalMove):
            reward[self.game.currentPlayer] = reward[self.game.currentPlayer] + 1
            reward[self.game.otherPlayer] = reward[self.game.otherPlayer] - 1
